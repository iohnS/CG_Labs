...
edaf80::Assignment2::Assignment2(WindowManager &windowManager) : mCamera(0.5f * glm::half_pi<float>(),
...
void edaf80::Assignment2::run()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (interpolate)
		{
			...
			//!        control points.
			if (curr_idx == control_point_locations.size())
			{
				curr_idx = 0;
			}
			...
			if (use_linear)
			{
				newPosition = interpolation::evalLERP(control_point_locations[curr_idx], control_point_locations[curr_idx + 1 % control_point_locations.size()], elapsed_time_s);
				//! \todo Compute the interpolated position
				//!       using the linear interpolation.
			}
			else
			{
				newPosition = interpolation::evalCatmullRom(control_point_locations[curr_idx], 
																		control_point_locations[curr_idx + 1 % control_point_locations.size()], 
																		control_point_locations[curr_idx + 2 % control_point_locations.size()], 
																		control_point_locations[curr_idx + 3 % control_point_locations.size()], 
																		catmull_rom_tension, 
																		elapsed_time_s);
				//! \todo Compute the interpolated position
